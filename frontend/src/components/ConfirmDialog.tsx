...
export function ConfirmDialog({ onConfirm, onCancel }: ConfirmDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b3332]/80 backdrop-blur-md p-4 animate-fade-in">
      <div className="w-full max-w-md bg-[#104443] border-2 border-[#d4af37] p-6 sm:p-8 shadow-2xl rounded-2xl relative overflow-hidden text-white">
        <div className="h-1.5 bg-[#d4af37] absolute top-0 inset-x-0" />
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-[#1a6866]">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="px-5 py-2.5 text-xs font-black uppercase tracking-widest bg-[#d4af37] hover:bg-[#b89528] text-[#0b3332] shadow-lg transition-all rounded-xl cursor-pointer border border-white/20 active:scale-95"
          >
            Confirmar
          </button>
        </div>
